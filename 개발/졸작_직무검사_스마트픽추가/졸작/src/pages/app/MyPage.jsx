...
import { useJobPreference } from '../../features/jobPreference.js'
...
export default function MyPage() {
  ...
  const liveJobMatches = preferenceResult.rankedRoles.slice(0, 3).map((role, index) => ({
    rate: Math.round(role.combinedScore),
    title: role.title,
    desc: `${role.description} 직무검사 ${role.assessmentScore}점과 채팅분석 ${role.chatScore}점을 60:40으로 반영한 ${index + 1}순위 추천입니다.`,
  }))
      ...
      <div key={activeTab} className="animate-fade-up">
        ...
        {activeTab === 'jobs' && (
          <div className="space-y-4">
            <div className="flex flex-col gap-2 rounded-card bg-ink p-5 text-white shadow-1 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-[12px] font-semibold text-white/65">실시간 통합 추천 분야</p>
                <p className="mt-1 text-[17px] font-bold">{preferenceResult.preferredCategory}</p>
              </div>
              <p className="text-[13px] text-white/75">
                직무검사 60% + 채팅분석 40% · 검사 {preferenceResult.answeredCount}/{preferenceResult.totalQuestions} 완료
              </p>
            </div>

            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {liveJobMatches.map((job) => (
              ...
              {!editingInfo ? (
                <dl className="mt-4 space-y-4">
                  ...
                  <div className="flex items-center justify-between border-b border-ink/8 pb-3">
                    ...
                    <dd className="text-[14px] font-semibold text-ink">{preferredField || '미설정'}</dd>
                  </div>
                  <div className="flex items-center justify-between">
                ...
                <form className="mt-4 space-y-4" onSubmit={handleProfileSubmit}>
                  <div>
                    <label htmlFor="profile-field" className="mb-1.5 block text-[13px] font-semibold text-ink">
                      선호 추천 분야
                    </label>
                    <input
                      id="profile-field"
                      ...
                      placeholder="예: IT · 백엔드 · AI"
                      className="w-full rounded-btn border border-ink/15 bg-canvas px-4 py-2.5 text-[14px] text-ink placeholder:text-taupe focus:border-coral focus:outline-none"
                    />
                  </div>
